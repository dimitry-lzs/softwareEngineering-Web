    ...
    >
        ...
        <thead>
            ...
            <th style={{ width: 48, textAlign: 'center', padding: '12px 6px' }}>
                ...
                onChange={(event) => {
                    setSelected(
                    event.target.checked ? rows.map((row) => row.time) : [],
                    );
                }}
                color={
                    selected.length > 0 || selected.length === rows.length
        ...
        <tbody>
            ...
            <tr key={row.date}>
                ...
                <Checkbox
                    size="sm"
                    checked={selected.includes(row.date)}
                    color={selected.includes(row.date) ? 'primary' : undefined}
                    onChange={(event) => {
                    setSelected((dates) =>
                        event.target.checked
                        ? dates.concat(row.date)
                        : dates.filter((itemDate) => itemDate!== row.date),
                    );
                    }}
